...
--2
-- A
SELECT s.date, e.first_name, e.last_name, s.item FROM sales s JOIN employees e ON s.employee = e.id WHERE employee = 111; 
-- B
...
-- B
SELECT concat(employees.first_name, " ", employees.last_name) AS "sales person", COUNT(sales.id) FROM sales JOIN employees ON sales.employee = employees.id GROUP BY employees.first_name, employees.last_name ORDER BY COUNT(sales.id) DESC; 
--4
-- A
